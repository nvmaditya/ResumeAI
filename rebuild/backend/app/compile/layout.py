"""Valid multi-page PDF layout fallback (stdlib only).

Not full TeX fidelity — produces real %PDF bytes, never corrupt stubs.
# ponytail: Helvetica layout only; tectonic when binary available.
"""
...
def structured_to_blocks(data: dict[str, Any] | None, title: str) -> list[tuple[str, str]]:
    ...
    if work:
        ...
        for w in work:
            head = " — ".join(
                x for x in [w.get("position") or "", w.get("company") or w.get("name") or ""] if x
            ) or "Role"
    ...
    if edu:
        ...
        for e in edu:
            line = ", ".join(
                x
                for x in [
                    e.get("degree") or e.get("studyType") or "",
                    e.get("area") or "",
    ...
    if skills:
        ...
        for s in skills:
            ...
            blocks.append(("body", f"{s.get('name') or 'Skills'}: {kw or ''}".strip(": ")))

    projects = data.get("projects") or []
    if projects:
        blocks.append(("h1", "Projects"))
        for p in projects:
            blocks.append(("body", str(p.get("name") or "Project")))
            if p.get("description"):
                blocks.append(("body", str(p["description"])))
    return blocks
...
def render_layout_pdf(
    *,
    ...
    if not pdf.startswith(b"%PDF") or len(pdf) < 200:
        raise RuntimeError("layout engine produced invalid PDF")
    return pdf
