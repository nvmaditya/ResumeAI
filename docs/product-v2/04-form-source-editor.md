...
Capture structured content **before** generate; edit LaTeX **after** (or from the start on New LaTeX). Support coach highlights and diagnostics jumps.
...
New AI resumes show full form. Legacy template meta may hide fields—rebuilds must not require a user template picker.
...
- Coach: strong highlight on selected hunk finds; dim optional on unselected.  
- Jump to diagnostic line; focus hunk from proposal list.  
...
| Coach hunks | Diff strip + highlights on source |
...
3. Source supports coach apply workflow.  
4. Contact fields on form, not Settings-only.
