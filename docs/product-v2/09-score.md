...
Overall + category scores with evidence so users prioritize edits and ground the coach. **Always manual.**

## Trigger

Toolbar **Check score** → after a finished job: **Re-check score**.  
Optional JD from coach panel (≤4000, sanitized). Dirty → save first.
...
- Overall /100  
- Categories (name, score, evidence; deductions/suggestions in full payload for coach)  
...
| After coach apply | **Old** score until Re-check |
...
- [ ] Stepper states  
- [ ] Re-check label after first job  
- [ ] No auto-score after apply  
